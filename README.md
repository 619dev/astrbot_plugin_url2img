...
对于某些 OpenAI 兼容服务，图片生成结果可能不是文本，而是放在原始响应的 `choices[0].img_urls` 中。AstrBot v4.26.2 会把这种“content 为空但 img_urls 有值”的响应判定为无可用输出。本插件会在加载时安装一个很小的兼容补丁，在 AstrBot 判空之前把这类 `img_urls` 注入为 URL 文本，再由本插件转换成图片消息，避免图片生成成功却不发图或留下误导性的空输出错误日志。该补丁会关闭 OpenAI provider 的请求重试，并把失败转换为终止响应，避免 AstrBot 将同一条生图命令继续提交给回退模型。
